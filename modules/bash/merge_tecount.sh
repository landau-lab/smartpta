...
awk 'NR > 1 {print $1}' "$first_file" > "$output_file"
...
while read -r file; do
    # Append the counts
    paste "$output_file" <(awk 'NR > 1 {print $2}' "$file") > temp && mv temp "$output_file"
    # Add the trimmed filename to the header, removing ".cntTable"
    filename=$(basename "$file" .cntTable)
